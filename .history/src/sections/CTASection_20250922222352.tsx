...
const CTASection: React.FC<CTASectionProps> = ({ data, theme, images }) => {
  ...
  // Handle mouse movement and scroll effects with improved parallax
  useEffect(() => {
    setIsLoaded(true);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Calculate mouse position even when outside the section for smoother transitions
      // This creates a more natural parallax effect that doesn't abruptly stop at section boundaries
      ...
      requestAnimationFrame(() => {
        ...
        // Reduce the movement intensity for more subtle effect with variable depth
        setMousePosition({
    ...
    const handleScroll = () => {
      ...
      // Calculate how far the section is through the viewport as a percentage
      const sectionProgress = 1 - (rect.top / viewportHeight);
      
      // Only update scroll value when section is approaching or in viewport
      // Extended range for smoother transition into view
      if (rect.top < viewportHeight + 300 && rect.bottom > -300) {
        // Use requestAnimationFrame for smoother animation
        requestAnimationFrame(() => {
          // Apply non-linear easing for more natural parallax
          const easedScrollValue = Math.min(1, Math.max(0, sectionProgress)) * window.scrollY;
          setScrollY(easedScrollValue);
        });
      }
    };
    ...
    <>
      ...
      >
        ...
        <div 
          ...
          style={{ 
            transform: `translate3d(0, ${scrollY * 0.08}px, 0) scale(${1 + Math.abs(scrollY) * 0.0001})`,
        ...
        >
          ...
          {/* Overlay with gradient - parallax in opposite direction for depth */}
          <div 
        ...
        <div 
          className="absolute top-0 right-0 w-96 h-96 bg-white opacity-5 rounded-full blur-3xl pulse-glow will-change-transform"
          style={{ 
            transform: `translate3d(${mousePosition.x * 0.15}px, ${mousePosition.y * 0.15 + scrollY * -0.02}px, 0)`,
            transition: 'transform 0.6s cubic-bezier(0.23, 1, 0.32, 1)'
          }}
        ></div>
        <div 
          className="absolute bottom-0 left-0 w-[30rem] h-[30rem] bg-white opacity-3 rounded-full blur-3xl pulse-glow will-change-transform"
          style={{ 
            transform: `translate3d(${mousePosition.x * -0.08}px, ${mousePosition.y * -0.08 + scrollY * 0.03}px, 0)`,
            animationDelay: '2s',
        ...
        <div 
          className="absolute top-[20%] left-[10%] w-24 h-24 rounded-full bg-white opacity-10 blur-xl floating-shape will-change-transform" 
          style={{
            animationDelay: '0.5s', 
            transform: `translate3d(${mousePosition.x * 0.04}px, ${mousePosition.y * 0.04 + scrollY * -0.05}px, 0)`,
            transition: 'transform 1.2s cubic-bezier(0.23, 1, 0.32, 1)'
          }}
        ></div>
        <div 
          className="absolute bottom-[30%] right-[15%] w-32 h-32 rounded-full bg-white opacity-10 blur-xl floating-shape will-change-transform" 
          style={{
            animationDelay: '1.2s', 
            transform: `translate3d(${mousePosition.x * -0.03}px, ${mousePosition.y * -0.03 + scrollY * 0.04}px, 0)`,
